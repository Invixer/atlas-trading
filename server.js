// ATLAS v2 - Production Trading Engine with proper static file serving
...
app.use(express.static(path.join(__dirname))); // Serve ALL files from current directory
app.use(express.static('.')); // Fallback

// If dashboard.html not found by static, serve it directly
app.get('/dashboard.html', (req, res) => {
  const filePath = path.join(__dirname, 'dashboard.html');
  if (fs.existsSync(filePath)) {
    res.sendFile(filePath);
  } else {
    res.status(404).send('dashboard.html not found at ' + filePath);
  }
});
...
function evaluateStrategy() {
  ...
  watchlist.forEach(sym => {
    ...
    const hasUpMomentum = momentum > 0.001;
    const hasDownMomentum = momentum < -0.001;
    ...
    if ((hasUpMomentum || hasDownMomentum) && goodPrice && haveEnoughCash) {
      const qty = Math.floor(portfolio.cash * 0.5 / currentPrice);
...
function checkExits() {
  watchlist.forEach(sym => {
    ...
    portfolio.positions[sym] = portfolio.positions[sym].filter(pos => {
      ...
      const shouldTakeProfit = pnl >= 0.015;
      const shouldStopLoss = pnl <= -0.01;
...
server.listen(PORT, () => {
  ...
  console.log(`[ATLAS] Dashboard: http://localhost:${PORT}/dashboard.html`);
  console.log(`[ATLAS] API: http://localhost:${PORT}/api/portfolio`);
});
